"""AI providers for the Privet API."""
...
class OpenRouterProvider(AIProvider):
    ...
    def __init__(self, api_key: str, model: str = "openai/gpt-3.5-turbo", expense_tracker=None):
        ...
        self.headers = {
            ...
            "HTTP-Referer": "https://github.com/privet-api",
            "X-Title": "Privet API",
        }
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            if "choices" in data and len(data["choices"]) > 0:
                                ...
                                # Log usage and track expenses if available
                                if "usage" in data:
                                    usage = data["usage"]
                                    ...
                                    logger.debug(
                                        f"OpenRouter usage - Prompt: {prompt_tokens}, "
                                        f"Completion: {completion_tokens}, "
                                        f"Total: {total_tokens}"
                                    )
                                    
                                    # Track expenses if expense tracker is available
                                    if self.expense_tracker:
                                        try:
                                            await self.expense_tracker.track_llm_usage(
                                                user_id=user_id,
                                                provider="openrouter",
                                                model=self.model,
...
class MockAIProvider(AIProvider):
    ...
    async def get_completion(self, prompt: str) -> str:
        """Get mock completion for a prompt."""
