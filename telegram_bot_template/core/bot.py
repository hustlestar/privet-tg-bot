...
from telegram_bot_template.handlers.voice_handler import VoiceHandler
...
class TelegramBot:
    ...
    async def setup(self) -> None:
        ...
        try:
            ...
            self.db_manager = DatabaseManager(self.config.database_url, auto_migrate=self.config.auto_migrate)
            await self.db_manager.setup()
            logger.info("Database manager initialized")

            # Initialize DAOs
            self.user_dao = UserDAO(self.db_manager.pool)
            logger.info("DAO objects initialized")
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            async with self.app:
                ...
                # Enhanced maintainer notification
                startup_message = f"🚀 **{self.config.bot_name}** v{self.config.bot_version}\n\n"
                startup_message += "✅ Bot started successfully!\n\n"
                startup_message += "**Services Status:**\n"
                startup_message += f"• Database: {'✅ Connected' if self.db_manager else '❌ Failed'}\n"
                startup_message += f"• AI Provider: {'✅ ' + self.config.openrouter_model if self.ai_provider else '❌ Not configured'}\n"
                startup_message += f"• Voice STT: {'✅ Ready' if self.config.openai_api_key else '⚠️ Not configured'}\n"
                startup_message += f"• Voice TTS: {'✅ Ready' if self.config.elevenlabs_api_key else '⚠️ Not configured'}\n"
                startup_message += f"• Support Bot: {'✅ Enabled' if self.support_bot else '❌ Disabled'}\n\n"
                startup_message += "🎙️ AI Companion features active!"
                
                await self._notify_maintainer(startup_message)
    ...
    def _add_handlers(self) -> None:
        ...
        self.app.add_handler(MessageHandler(filters.VOICE, self.voice_handler.handle_voice))
    ...
    async def _error_handler(self, update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # Notify maintainer about critical errors
        if self.config.MAINTAINER_CHAT_ID:
            error_message = f"⚠️ **Error in {self.config.bot_name}**\n\n"
            error_message += f"**Error Type:** {type(context.error).__name__}\n"
            ...
            try:
                ...
                logger.error(f"Failed to send error notification to maintainer: {e}")
    ...
    async def _notify_maintainer(self, text: str) -> None:
        ...
        if self.config.MAINTAINER_CHAT_ID and self.app:
            try:
                ...
                logger.info(f"Sent notification to maintainer")
            except Exception as e:
                ...
                try:
                    ...
                    logger.info(f"Sent notification to maintainer (plain text)")
                except Exception as e2:
                    logger.error(f"Failed to send notification to maintainer {self.config.MAINTAINER_CHAT_ID}: {e2}")
    ...
    async def get_stats(self) -> dict:
        ...
        if self.user_dao:
            db_stats = await self.user_dao.get_stats()
    ...
    async def shutdown_gracefully(self) -> None:
        ...
        # Notifications
        shutdown_message = f"🛑 **{self.config.bot_name}** v{self.config.bot_version}\n\n"
        shutdown_message += "⚠️ Bot is shutting down...\n\n"
        shutdown_message += "**Shutdown Reason:** "
        if self.shutting_down_event and self.shutting_down_event.is_set():
            shutdown_message += "Signal received (SIGTERM/SIGINT)\n"
        else:
            shutdown_message += "Manual shutdown\n"
        shutdown_message += "\n📊 Session Statistics will be available on next start."
        
        await self._notify_maintainer(shutdown_message)
        ...
        try:
            # Check if application has been initialized (PTB doesn't have initialized attribute)
            # We can check if it's running or has been started instead
            if hasattr(self.app, 'shutdown'):
                ...
                logger.info("Application shutdown not available.")
        except RuntimeError as e:
        ...
        if self.db_manager:
            ...
            logger.warning("Database manager not found.")
