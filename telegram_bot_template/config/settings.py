...
class BotConfig:
    ...
    openai_api_key: Optional[str] = None  # For Whisper STT and OpenAI TTS
    
    # TTS Configuration
    tts_provider: str = "elevenlabs"  # Options: elevenlabs, openai, google, amazon
    tts_voice: Optional[str] = None  # Provider-specific voice ID
    tts_language: str = "en-US"  # Language for TTS
    tts_model: Optional[str] = None  # Model selection (provider-specific)
    
    # Provider-specific API keys
    elevenlabs_api_key: Optional[str] = None  # For ElevenLabs TTS
    google_tts_api_key: Optional[str] = None  # For Google Cloud TTS (path to service account JSON)
    amazon_polly_api_key: Optional[str] = None  # For Amazon Polly (format: "access_key,secret_key,region")
    ...
    def from_env(cls) -> "BotConfig":
        ...
        amazon_polly_api_key = os.getenv("AMAZON_POLLY_API_KEY")
    ...
    def __str__(self):
        """String representation of the configuration, redacting sensitive info."""
        ...
        return (
            ...
            f"  Auto-migrate DB: {'✅' if self.auto_migrate else '❌'}\n"
            f"  AI Support: {'✅' if self.has_ai_support else '❌'}\n"
            f"    - AI Model: {self.openrouter_model if self.has_ai_support else 'N/A'}\n"
            f"  Support Bot: {'✅' if self.has_support_bot else '❌'}\n"
            f"    - Support Chat ID: {self.support_chat_id if self.has_support_bot else 'N/A'}\n"
            f"    - Maintainer Chat ID: {self.MAINTAINER_CHAT_ID or 'Not set'}\n"
            f"  Tokens:\n"
            f"    - Bot Token: {redact(self.bot_token)}\n"
            f"    - OpenRouter API Key: {redact(self.openrouter_api_key)}\n"
            f"    - OpenAI API Key: {redact(self.openai_api_key)}\n"
            f"    - ElevenLabs API Key: {redact(self.elevenlabs_api_key)}\n"
            f"    - Support Bot Token: {redact(self.support_bot_token)}\n"
